  ...
  messages: {
    ...
    for(let node of nodeDataArray){
      ...
      if(node.name != node.type || node.type == 'Action' || node.type == 'Condition' || node.type == 'Composite' || node.type == 'Decorator'){
            ...
            customResetInEditor +="  if(!this.getComponent(\'"+node.name+"\')){\n" +
                                  "     let t = this.addComponent(\'"+node.name+"\');\n" +
                                  "	    if(t[\'resetInEditor\']){\n" +
                                  "		     t[\'resetInEditor\']();\n" +
                                  "	    }\n" +
                                  "   }\n";
            customNodeString += "let "+node.name+"Component=self.getComponent(\'"+node.name+"\');\n" +
                                "let "+node.name+" = b3.Class(b3."+node.type+");\n" +
            ...
            customNodeString += "}\n" + 
                                ...
                                "           return "+node.name+"Component.enter(tick,b3,this);\n" +
                                "}\n" + 
                                node.name+".prototype.open = function(tick) {\n" +
                                "           return "+node.name+"Component.open(tick,b3,this);\n" +
                                "}\n" + 
                                node.name+".prototype.tick = function(tick) {\n" +
                                "           return "+node.name+"Component.tick(tick,b3,this);\n" +
                                "}\n" + 
                                node.name+".prototype.close = function(tick) {\n" +
                                "           return "+node.name+"Component.close(tick,b3,this);\n" +
                                "}\n" + 
                                node.name+".prototype.exit = function(tick) {\n" +
                                "           return "+node.name+"Component.exit(tick,b3,this);\n" +
                                "}\n";
      }
    }
